...
void main() {
  ...
  vec3 auroraColor = mix(vec3(uDarkTheme), rampColor, t);
...
`,
  ...
  onMount(() => {
    ...
    const geomAttrs = (geometry as unknown as { attributes: Record<string, unknown> })
      .attributes
    ...
    const program = new Program(gl, {
      ...
      uniforms: {
        ...
        uDarkTheme: { value: isDarkTheme ? 0 : 1 },
      },
    })
    ...
    const update = (t: number) => {
      ...
      program.uniforms.uDarkTheme.value = isDarkTheme ? 0 : 1
...
<div bind:this={ctn} class="aurora-bg w-full h-full" {style}></div>
